...
App.post(async (req, res) => {
  ...
  try {
    ...
    // Send email
    const emailContent = signupEmail(user.email); // This should return the HTML content of the email
    const emailResponse = await axios.post("https://aquakart.co.in/api/send-email", {
      email: user.email,
    ...
    res.status(200).json({
      ...
      emailMessage: emailResponse.data.message,
    });
