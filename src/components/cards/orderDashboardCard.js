    ...
    return (
        <div className="card shadow-lg mb-3">
            <div className="card-body">
                <h3 className={paymentStatus === "Paid" ? "text-success" : "text-danger"}>{paymentStatus}</h3>
                <h5 className="text-success"><AquaCurrencyFormat amount={totalAmount}/></h5>
                <hr/>
                ...
                <div>
                    ...
                    {items.map(item => (
                        <div key={item._id} className="text-muted">
                            <p>{item.name} - {item.quantity} x {<AquaCurrencyFormat amount={item.price}/>}</p>
                        </div>
                    ))}
                </div>
            </div>
